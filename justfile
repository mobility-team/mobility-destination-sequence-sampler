...
audit-global-quality per_stratum='1' max_states='200000': build-release
    mamba run -n mobility-destination-sequence-sampler python -m experiments.analysis.compare_bidirectional_top_k_grand_geneve --contexts-per-stratum {{per_stratum}} --top-k 10 --oracle-depth 100 --max-states {{max_states}} --frontier-width 40 --proposal-limit-per-source 16 --continuation-state-limit 1 --continuation-proposal-limit 1; exit $LASTEXITCODE
...
audit-deep-top-k per_stratum='2' max_states='500000': build-release
    mamba run -n mobility-destination-sequence-sampler python -m experiments.analysis.compare_bidirectional_top_k_grand_geneve --contexts-per-stratum {{per_stratum}} --top-k 10 --oracle-depth 10 --max-states {{max_states}} --frontier-width 40 --proposal-limit-per-source 16 --continuation-state-limit 1 --continuation-proposal-limit 1; exit $LASTEXITCODE
