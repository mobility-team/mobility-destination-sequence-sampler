...
| Cross-context exact factor-map sharing | Rejected | Lock-free scoring-shape groups preserved identical outputs and semantic work and cut full-workload map scans about 15% (3.13B->2.65B), but the locked two-block gate improved factor-map CPU only 0.8% while aggregate Rust regressed 1.7% and wall time regressed 4.3%. Large cache lookup/state and grouping costs consumed the saved scans. |
| Composed factor-map result cache | Rejected | Exact local-boundary keys hit on 25% of 1,000-context queries and looked promising in fast screens, but repeated 20,000-context counterbalanced runs did not produce a stable 3% Rust gain. Obvious OS outliers hit both labels; the remaining observations implied only a roughly 1--2% effect with sign changes, too small to justify another cache. |
| K-wide beam for isolated variables | Rejected | Narrowing the width-40 beam to requested K only when fixed destinations isolate every variable and no anchor repeats preserved exact quality on a locked 396-context audit (post-stratified Mass@10 0.864136 and 24 zero-mass cases for both). A 1,000-context screen suggested a 16% Rust gain, but the independent 20,000-context two-block gate measured paired wall -1.5% (range -5.3% to +2.2%), aggregate Rust +0.0%, and factor-map CPU +0.2%. The extra policy was removed. |
| Approximate factor-map logarithm | Rejected | Proposal-only logarithms left the small exact cohort and 1,000-context output fingerprint unchanged, but did not accelerate the Windows release kernel. A seven-term range-reduced polynomial regressed factor-map CPU 4.9%; native `f32::ln` changed it by 0.0% and aggregate Rust by +0.4%. Exact final scoring was never changed, and the experimental policy was removed. |
...
| Static attraction/OD successor prefixes | Rejected | Restricting each oracle parent to capacity-ranked and cheapest-outgoing prefixes repaired false-infeasible context 19665 at width 16 with `Mass@10` 0.776 and 1,029 child evaluations. On zero-mass context 24578, widths 32/48/64 reached mass 0.113/0.429/1.000 with 76,754/227,572/429,979 children. Exact recovery therefore needed a width that cut the baseline's at-least 4.05M evaluated children only 9.4x, missing the 20x gate; a fixed prefix merely relocates the support cutoff. |
